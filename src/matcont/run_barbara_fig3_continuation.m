...
branches = repmat(struct('shape', [], 'seedTau', [], 'label', '', ...
    'points', [], 'specialPoints', [], 'runs', []), numel(seeds), 1);
...
if ~isfield(options, 'cycleTolerance')
    options.cycleTolerance = 5e-2;
end
if ~isfield(options, 'multiplierTolerance')
    options.multiplierTolerance = 1e-3;
end
if ~isfield(options, 'funTolerance')
    options.funTolerance = 1e-6;
end
if ~isfield(options, 'varTolerance')
    options.varTolerance = 1e-6;
end
if ~isfield(options, 'testTolerance')
    options.testTolerance = 1e-5;
end

if options.quickMode
    defaults = struct('ntst', 12, 'ncol', 4, 'maxNumPoints', 55, ...
        'initStepSize', 0.025, 'maxStepSize', 0.12, 'minStepSize', 1e-4, ...
        'adapt', 3, 'seedTotalTime', 500, 'seedSampleCount', 4500, ...
        'cycleSampleCount', 800);
else
    defaults = struct('ntst', 20, 'ncol', 4, 'maxNumPoints', 130, ...
        'initStepSize', 0.02, 'maxStepSize', 0.08, 'minStepSize', 1e-5, ...
        'adapt', 3, 'seedTotalTime', 800, 'seedSampleCount', 6500, ...
        'cycleSampleCount', 1100);
end
