...
for k = 1:numel(branches)
    ...
    plot_stability_segments(ax, tau, phase, stable, color, branches(k).label);
end
...
known = isfinite(stable);
if ~any(known)
    plot(ax, tau, phase, '-', 'Color', color, 'LineWidth', 1.5, ...
...
for idx = 2:(numel(tau) + 1)
    changed = idx > numel(tau) || stable(idx) ~= stable(startIdx) || ...
        ~isfinite(stable(idx));
    if changed
